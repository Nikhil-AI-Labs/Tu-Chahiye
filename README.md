...
│   ├── fig/                23 computed figures, generated by tools/make-figures.py
...
│   ├── make-figures.py     the 23 computed figures, simulated rather than drawn
...
**87** diagrams, **1038** typeset expressions, **20** generated PDFs and **13** original
scans.
...
**Computed** — `assets/fig/`, 23 files, made by `tools/make-figures.py`. These are
...
even though a bare `a` there is the roll-off factor.
...
An inline `<code>` span that is really an expression becomes inline maths.
...
Source code, bit patterns, ASCII tables, character diagrams, anything carrying markup it
cannot read, and anything the parser cannot read with confidence. A monospace block is a
